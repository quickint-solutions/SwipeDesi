...
const shopSingleHttpRequest = {
  ...
  addToWishlist: (userId: number, productId: number, action: string) =>
    apiRequests.get(`api/shopsingleproduct/ManageWishList?userid=${userId}&productid=${productId}&action=${action}`),
};
