                        ...
                        <div className="col-md-12 position-relative">
                            <div className="header-inner-title">
                                <div className="section-title">
                                    ...
                                    <h2 className="title text-white"><strong>Shop Cart</strong></h2>
                                </div>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <tbody>
                                            {cartDetail?.length > 0 ?
                                                cartDetail.map((value: any, key: number) => (
                                                    <tr key={key}>
                                                        <td className="product-remove"><a href="javascript:void(0)" onClick={() => removeItemFromCart(value)}><i className="fa-solid fa-xmark"></i></a></td>
                                                        <td className="product-thumbnail"><a href="javascript:void(0)"><img src={process.env.REACT_APP_IMAGE_BASE_URL + value.imagePath?.replace("~", "")} alt="" /></a></td>
                                                        <td className="product-name"><a href="javascript:void(0)">{value.productName}</a></td>
                                                        <td className="product-price"><span className="amount">${value.price?.toFixed(2)}</span></td>
                                                        <td className="product-quantity">
                                                            <form>
                                                                <div>
                                                                    <input type="number" className="form-control" min={1} id="number" value={value.quantity} onChange={(e) => handleCartQuantity(Number(e.target.value), value)} />
                                                                </div>
                                                            </form>
                                                        </td>
                                                        <td className="product-subtotal"><span className="subtotal">${(value.quantity * value.price).toFixed(2)}</span></td>
                                                    </tr>
                                                ))
                                                :
                                                <tr>
                                ...
                                {cartDetail?.length > 0 ?
                                    <div className="actions">
                                        <div className="coupon mt-2 mt-lg-0 mt-md-0 mt-sm-0">
                                            <form>
                                                <input type="text" name="coupon_code" className="input-text" id="coupon_code" value={couponCode} placeholder="Coupon code" onChange={(e) => setCouponCode(e.target.value)} />
                                                {couponData ?
                                                    <a className="btn btn-primary" href="javascript:void(0)" onClick={() => removeCoupon()}>Remove coupon</a>
                                                    :
                                                    <a className="btn btn-primary" href="javascript:void(0)" onClick={() => applyCoupon()}> Apply coupon</a>
                                                }
                                        ...
                                        <div className="update-cart mt-3 mt-md-0"> <a className="btn btn-secondary" href="javascript:void(0)" onClick={() => updateCart()}> Update cart</a></div>
                                    </div>
                                    : ""
                                }
                            </div>
                            ...
                            <div className="cart-total-sidebar">
                                <div className="cart-totals widget">
                                        ...
                                        <table className="table table-bordered">
                                            <tbody>
                                                <tr className="cart-subtotal">
                                                    <th>Subtotal</th>
                                                    <td><span className="subtotal">${cartDetail?.length > 0 ? cartTotalData?.Subtotal?.toFixed(2) : "0.00"}</span></td>
                                                </tr>
                                                <tr className="shipping">
                                                    ...
                                                    <td>
                                                        ...
                                                        <p className="mb-2">Shipping to <strong>CA.</strong></p>
                                                        <a href="#">Change address <i className="fas fa-shopping-cart ms-2"></i></a>
                                                    </td>
                                                </tr>
                                    ...
                                    <div className="text-center">
                                        <a href="javascript:void(0)" className="btn btn-primary checkout-button" onClick={() => navigate("/Checkout")}>Proceed to checkout</a>
                                    </div>
                                </div>
                            </div>
                        </div>
